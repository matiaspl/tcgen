...
fps=$1
debug="-nostats -loglevel 0" # silent
...
ffplay $debug -fs -f lavfi -i "smptehdbars=r=$fps:s=1280x720" -vf "\
        ...
        [back+grid][box]overlay=x='32*mod(mod(n-1,$fps),10)':y='32*floor(mod(n-1,$fps)/10)' \
